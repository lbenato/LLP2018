...
if RunLocal:
    ...
    isVBF             = options.PVBF
    isggH             = options.PggH
    isPuppi           = True#False#options.PisPuppi
...
# Jet collection parameters
jet_collection = "Puppi" if isPuppi else "CHS"#"Puppi"
slimmed_jet_collection = "Puppi" if isPuppi else "" #"Puppi"
jec_collection_name = "Puppi" if isPuppi else "chs" #"Puppi"
pfcand = "puppi" if isPuppi else "pfCHS" #puppi

#Jet labels
if isCalo:
   chosen_JEC = "AK4PF" + jec_collection_name
   chosen_jet_source = 'ak4PFJets' + jet_collection + 'Custom'#'ak4PFJetsCHSCustom'
   chosen_label = 'Reclustered'
   chosen_pfcand = pfcand
   chosen_jets = "patJets"+ chosen_label
   pt_AK4 = 5
   print "#-----------------------#"
   print "#   Jet collections:    #", chosen_pfcand, chosen_jet_source, chosen_JEC, chosen_jets
   print "#-----------------------#"
else:
   chosen_jets = "slimmedJets"+slimmed_jet_collection
   pt_AK4 = 15
...
if isCalo:
   ...
   process.ak4PFJetsPuppiCustom = ak4PFJetsPuppi.clone(src = "puppi", doAreaFastjet = True, jetPtMin = pt_AK4)
   task.add(process.ak4PFJetsPuppiCustom)

   #chosen_jet_source = 'ak4PFJetsPuppiCustom'
...
jetCorrectionsAK4 = (chosen_JEC, ['L1FastJet', 'L2Relative', 'L3Absolute'], 'None')
...
process.patJetCorrFactorsFinal.payload = chosen_JEC
...
#'HLT_VBF_DisplacedJet40_VTightID_Hadronic_v', 'HLT_VBF_DisplacedJet40_VVTightID_Hadronic_v'#,'HLT_AK4PFJet30_v7'
    ...
    allJetSet = cms.PSet(
        ...
        jecUncertaintyDATA = cms.string('data/%s/%s_Uncertainty_AK4PF%s.txt' % (JECstring, JECstring, jec_collection_name)),#updating
        jecUncertaintyMC = cms.string('data/Summer16_23Sep2016V3_MC/Summer16_23Sep2016V3_MC_Uncertainty_AK4PF%s.txt' % (jec_collection_name)),#updating
        jecCorrectorDATA = cms.vstring(#updating
            'data/%s/%s_L1FastJet_AK4PF%s.txt' % (JECstring, JECstring, jec_collection_name),
            'data/%s/%s_L2Relative_AK4PF%s.txt' % (JECstring, JECstring, jec_collection_name),
            'data/%s/%s_L3Absolute_AK4PF%s.txt' % (JECstring, JECstring, jec_collection_name),
            'data/%s/%s_L2L3Residual_AK4PF%s.txt' % (JECstring, JECstring, jec_collection_name),
        ),
        jecCorrectorMC = cms.vstring(#updating!!!
            'data/Summer16_23Sep2016V3_MC/Summer16_23Sep2016V3_MC_L1FastJet_AK4PF%s.txt' % (jec_collection_name),
            'data/Summer16_23Sep2016V3_MC/Summer16_23Sep2016V3_MC_L2Relative_AK4PF%s.txt' % (jec_collection_name),
            'data/Summer16_23Sep2016V3_MC/Summer16_23Sep2016V3_MC_L3Absolute_AK4PF%s.txt' % (jec_collection_name),
        ),
        ...
        jerNameRes = cms.string('data/JER/Spring16_25nsV10_MC_PtResolution_AK4PF%s.txt' % (jec_collection_name)),#v10 is the latest
        jerNameSf = cms.string('data/JER/Spring16_25nsV10_MC_SF_AK4PF%s.txt' % (jec_collection_name)),#v10 is the latest
    ),
    chsJetSet = cms.PSet(
        jets = cms.InputTag(jets_to_be_used),#(jets_after_btag_tools),#('updatedPatJetsTransientCorrected'+postfix),
        jetid = cms.int32(0), # 0: no selection, 1: loose, 2: medium, 3: tight
...
outFile = open("tmpConfig_Ntuplizer%s.py" % (jet_collection),"w")
