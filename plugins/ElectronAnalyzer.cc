...
ElectronAnalyzer::ElectronAnalyzer(const edm::ParameterSet& PSet, edm::ConsumesCollector&& CColl):
    ...
    EleVetoId(PSet.getParameter<std::string>("eleVetoId")),
    EleLooseId(PSet.getParameter<std::string>("eleLooseId")),
    EleMediumId(PSet.getParameter<std::string>("eleMediumId")),
    EleTightId(PSet.getParameter<std::string>("eleTightId")),
    EleHEEPId(PSet.getParameter<std::string>("eleHEEPId")),
    EleMVANonTrigMediumId(PSet.getParameter<std::string>("eleMVANonTrigMediumId")),
    EleMVANonTrigTightId(PSet.getParameter<std::string>("eleMVANonTrigTightId")),
    EleMVATrigMediumId(PSet.getParameter<std::string>("eleMVATrigMediumId")),
    EleMVATrigTightId(PSet.getParameter<std::string>("eleMVATrigTightId")),
...
//w//EleScaleSmearCorrectionName(PSet.getParameter<std::string>("eleScaleSmearCorrectionName")),
    ...
    Electron2Pt(PSet.getParameter<double>("electron2pt"))
{
    ...
    //w//eScaleSmearer = new EnergyScaleCorrection_class(EleScaleSmearCorrectionName);
...
std::vector<pat::Electron> ElectronAnalyzer::FillElectronVector(const edm::Event& iEvent) {
    ...
    iEvent.getByToken(EleVetoId, VetoIdDecisions);
    iEvent.getByToken(EleLooseId, LooseIdDecisions);
    iEvent.getByToken(EleMediumId, MediumIdDecisions);
    iEvent.getByToken(EleTightId, TightIdDecisions);
    iEvent.getByToken(EleHEEPId, HEEPIdDecisions);
    iEvent.getByToken(EleMVANonTrigMediumId, MVANonTrigMediumIdDecisions);
    iEvent.getByToken(EleMVANonTrigTightId, MVANonTrigTightIdDecisions);
    iEvent.getByToken(EleMVATrigMediumId, MVATrigMediumIdDecisions);
    iEvent.getByToken(EleMVATrigTightId, MVATrigTightIdDecisions);
    */
    ...
    //w//
    //w//if (isMC){
    //w//eScaleSmearer->doSmearings = true;
    //w//}
    //w//else{
    //w//eScaleSmearer->doScale = true;
    //w//}
    ...
    for(std::vector<pat::Electron>::const_iterator it=EleCollection->begin(); it!=EleCollection->end(); ++it) {
        ...
        bool isPassVeto = el.electronID(EleVetoId);//(*VetoIdDecisions)[elRef];
	bool isPassLoose = el.electronID(EleLooseId);//(*LooseIdDecisions)[elRef];
	bool isPassMedium = el.electronID(EleMediumId);//(*MediumIdDecisions)[elRef];
	bool isPassTight = el.electronID(EleTightId);//(*TightIdDecisions)[elRef];
	bool isPassHEEP = el.electronID(EleHEEPId);//(*HEEPIdDecisions)[elRef];
	bool isPassMVANonTrigMedium = el.electronID(EleMVANonTrigMediumId);//(*MVANonTrigMediumIdDecisions)[elRef];
	bool isPassMVANonTrigTight = el.electronID(EleMVANonTrigTightId);//(*MVANonTrigTightIdDecisions)[elRef];
	bool isPassMVATrigMedium = el.electronID(EleMVATrigMediumId);//(*MVATrigMediumIdDecisions)[elRef];
	bool isPassMVATrigTight = el.electronID(EleMVATrigTightId);//(*MVATrigTightIdDecisions)[elRef];
	...
	//not sure, to be checked later:
        ...
        //w//TRandom3 * rgen_ = new TRandom3(0);//not needed?

        if(isMC){
	    //w//float smear = rgen_->Gaus(1, sigma);
	    //w//float smearUp = rgen_->Gaus(1, error_sigma_u);
	    //w//float smearDown = rgen_->Gaus(1, error_sigma_d);
  	    el.addUserFloat("SScorr",               scale);//smear);
            el.addUserFloat("ptSScorr",             el.pt()*scale);//smear);
            el.addUserFloat("ptSScorrUncUp",        el.pt()*(1+error_scale));//smearUp);
	    el.addUserFloat("ptSScorrUncDown",      el.pt()*(1-error_scale));//smearDown);
	    el.addUserFloat("energySScorr",         el.energy()*scale);//smear);
	    el.addUserFloat("energySScorrUncUp",    el.energy()*(1+error_scale));//smearUp);
	    el.addUserFloat("energySScorrUncDown",  el.energy()*(1-error_scale));//smearDown);
        }
        else{
 	    //question: not applied on data??
	    el.addUserFloat("SScorr",                 0.);//scale);
	    el.addUserFloat("ptSScorr",               0.);//el.pt()*scale);
	    el.addUserFloat("ptSScorrUncUp",          0.);//el.pt()*(1+error_scale));
	    el.addUserFloat("ptSScorrUncDown",        0.);//el.pt()*(1-error_scale));
	    el.addUserFloat("energySScorr",           0.);//el.energy()*scale);
	    el.addUserFloat("energySScorrUncUp",      0.);//el.energy()*(1+error_scale));
	    el.addUserFloat("energySScorrUncDown",    0.);//el.energy()*(1-error_scale));            
        }
