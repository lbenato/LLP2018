...
TriggerStudies::TriggerStudies(const edm::ParameterSet& iConfig):
    ...
    WriteOnlyAtLeastOneL1BitFiredEvents(iConfig.getParameter<bool>("writeOnlyAtLeastOneL1BitFiredEvents")),
...

{
    ...
    l1GtUtils_ = new l1t::L1TGlobalUtil(TriggerPSet,consumesCollector());
...
TriggerStudies::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
      ...
      if (l1htbit)
        {
          ...
          AtLeastOneL1Bit = true;
        }
    }
    if (!AtLeastOneL1Bit && WriteOnlyAtLeastOneL1BitFiredEvents)            return;
    ...
    //   {
    // 	curr_deltaR_muon = fabs(reco::deltaR(IsoMu24_Vec.at(a).eta(),IsoMu24_Vec.at(a).phi(),MuonVect.at(0).eta(),MuonVect.at(0).phi()));
    ...
    //// Build candidate
    //if(e1 >= 0 && e2 >= 0) {
    //	theZ.addDaughter(TightElecVect.at(e1).charge() ? TightElecVect.at(e1) : TightElecVect.at(e2));
    ...
    */
    ...
    }//end if write one of the PFCandidate bools
...
void
TriggerStudies::calcNsubjettiness(const pat::Jet & jet, float & tau1_neutral, float & tau1_charged, float & tau2_neutral, float & tau2_charged, std::vector<fastjet::PseudoJet> & currentAxes) const
{
