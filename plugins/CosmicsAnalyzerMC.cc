// -*- C++ -*-
//
// Package:    Cosmics/CosmicsAnalyzerMC
// Class:      CosmicsAnalyzerMC
//
/**\class CosmicsAnalyzerMC CosmicsAnalyzerMC.cc Cosmics/CosmicsAnalyzer/plugins/CosmicsAnalyzerMC.cc
...
class CosmicsAnalyzerMC : public edm::one::EDAnalyzer<edm::one::SharedResources>  {
   public:
      explicit CosmicsAnalyzerMC(const edm::ParameterSet&);
      ~CosmicsAnalyzerMC();
  ...
  std::vector<GenPType> GenParticles;
...
//
CosmicsAnalyzerMC::CosmicsAnalyzerMC(const edm::ParameterSet& iConfig)
 :
  ...
  genParticlesToken_(consumes<std::vector<reco::GenParticle> >(iConfig.getUntrackedParameter<edm::InputTag>("genParticles")))
...
CosmicsAnalyzerMC::~CosmicsAnalyzerMC()
{
...
void
CosmicsAnalyzerMC::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   using namespace edm;
   ...
   */
   ...
   //GenParticles
   std::vector<reco::GenParticle> GenParticlesVect;
   for(reco::GenParticleCollection::const_iterator gen_it = genParticles->begin(); gen_it != genParticles->end();gen_it++) {
     reco::GenParticle genp = *gen_it;
     //std::cout << "Gen part id " << genp.pdgId() << " " << genp.status() << " " << genp.pt() << std::endl;
     if(genp.status()==1)
       {
	 GenParticlesVect.push_back(genp);
       }
   }
   nGenParticles = GenParticlesVect.size();
   for(unsigned int i =0; i< GenParticlesVect.size();i++) GenParticles.push_back(GenPType());
   for(unsigned int i =0; i< GenParticlesVect.size();i++) ObjectsFormat::FillGenPType(GenParticles[i], &GenParticlesVect[i]);
...
CosmicsAnalyzerMC::beginJob()
{
  ...
  tree -> Branch("nGenParticles", &nGenParticles, "nGenParticles/L");
...
void
CosmicsAnalyzerMC::endJob()
{
}

// ------------ method fills 'descriptions' with the allowed parameters for the module  ------------
void
CosmicsAnalyzerMC::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
DEFINE_FWK_MODULE(CosmicsAnalyzerMC);
